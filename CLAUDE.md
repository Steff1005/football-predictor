...
- **Одночасні матчі:** ESPN-події мапити масивом на хвилину старту + матч по назвах команд (інакше рахунок «перетікає»).
  Спільна логіка — `lib/match-espn.js` (`sameTeam`, `pickEspnEvent`); використовують і `live-scores`, і `sync-matches`.
  Без збігу назв повертати `null`, а не «перший-ліпший» кандидат.
- **Стрічка ESPN за лігою:** `live-scores` бере slug із `ESPN_SLUG[league_id]` (CL→`uefa.champions` тощо).
  Раніше був зашитий `fifa.world` → після ЧС лайв ЛЧ не знаходив жодного матчу.
- **Дублікати подій ESPN:** та сама подія приходить із загальної й датованої стрічок з різною свіжістю —
  дедуплікувати за `event.id`, лишаючи свіжішу (завершена > пізніша хвилина). Інакше «застигає» хвилина.
- **Статус не регресує:** `sync-matches` ніколи не повертає `finished` → `live/scheduled` (fd.org відстає
  від ESPN на хвилини; було: Ман Юнайтед—Сабах 10.09 finished→live→finished).
- **Клієнти лайву** (`LiveTab`, `LiveAllClient`) прибирають матч, якого немає у відповіді `/api/live-scores`
  (він уже фіналізований). Порожня відповідь валідна = завершились усі.
- **Зміна рахунку після нарахування:** `calcPredictions` ідемпотентний, тож `sync-matches` при зміні
  рахунку завершеного матчу скидає `is_calculated` і перераховує. Аудит: `scripts/audit-points.mjs [--fix]`.
- **Журнал рахунків:** тригери БД пишуть кожну зміну рахунку/статусу в `match_score_history`
  (+ `matches.updated_at`). Перегляд: `scripts/score-history.mjs [--suspicious] [--days N]`.
- **Довгі імена на Android:** імена учасників у рядках прогнозів — `line-clamp-2 break-words`, не `truncate`
  (Android масштабує системний шрифт сильніше за iOS → обрізало «Олександр Шляхтюк (П)»).
- **Нижня панель:** `BottomNav` має `md:hidden` — на ширині ≥768px (планшети, особливо горизонтально) її
  немає, показується верхнє меню як на ПК. Це за шириною вікна, не за типом пристрою.
