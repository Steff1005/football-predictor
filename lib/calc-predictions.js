...
import { generateMatchAnalysis } from './generate-match-analysis'
...
export async function calcPredictions(supabase, match, homeScore, awayScore) {
  ...
  // Auto-generate match analysis (fire-and-forget)
  const calculatedPreds = predictions.map(p => {
    const pts = calculatePoints(p.predicted_home, p.predicted_away, homeScore, awayScore)
    return { ...p, home_score: homeScore, away_score: awayScore, ...pts }
  })
  generateMatchAnalysis(supabase, { ...match, home_score: homeScore, away_score: awayScore }, calculatedPreds)
    .catch(e => console.error('generateMatchAnalysis failed:', e.message))
