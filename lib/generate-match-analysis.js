...
export async function generateMatchAnalysis(supabase, match, predictions) {
  ...
  const dupeNames = Object.entries(firstNameCount)
    .filter(([, n]) => n > 1)
    .map(([fn]) => {
      const names = Object.values(profileMap)
        .filter(p => p.first_name === fn)
        .map(p => fullName(p))
      return `НЕ пиши просто "${fn}" або "${fn} [прізвище]" — ЗАВЖДИ пиши: ${names.join(' або ')}`
    })

  const nameRules = [
    dupeNames.length
      ? `УВАГА — однакові імена: ${dupeNames.join('; ')}. Суфікс обов'язковий кожного разу, без винятків.`
      : '',
...
ВАЖЛИВО про проміжний рахунок: поки матч іде, учасники ще в грі. Якщо рахунок 1:1 в середині матчу — той, хто ставив на перемогу будь-якої команди, ще має шанс. "Вибуває" учасник лише якщо його результат стає математично неможливим (наприклад, ставив 1:0, а вже 0:2).
