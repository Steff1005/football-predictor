...
export async function generateMatchAnalysis(supabase, match, predictions) {
  ...
  // Fetch ESPN events first so we can build the prompt around them
  ...
  const firstParaInstruction = eventsBlock
    ? `Перший абзац: розкажи як пройшов матч — ОБОВ'ЯЗКОВО згадай авторів голів та хвилини (переклади імена на українську), хто тримав ініціативу, чим запам'ятався поєдинок.`
    : `Перший абзац: дві-три речення про сам матч — хто був сильнішим, як розвивалися події, чим запам'ятався.`

  const finalPrompt = `Ти — спортивний коментатор, пишеш короткий репортаж про футбольний матч для групи друзів-прогнозистів. Мова: українська, жива, але лаконічна. Суцільний текст без заголовків і списків. Рівно 2-3 абзаци.

Матч: ${match.home_team} – ${match.away_team}, рахунок ${match.home_score}:${match.away_score}.${eventsBlock}
