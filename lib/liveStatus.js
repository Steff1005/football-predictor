...
export function getGameTime(clock, wallElapsed, halftime) {
  if (clock) {
    // "90+7'" stoppage time
    const stoppage = clock.match(/^(\d+)\+(\d+)/)
    if (stoppage) {
      const base = parseInt(stoppage[1]), extra = parseInt(stoppage[2])
      const ge = base + extra
      return { elapsed: ge, fullTime: base >= 90 ? ge + 3 : 95 }
    }
    // "67'" normal time
    const normal = clock.match(/^(\d+)/)
    if (normal) return { elapsed: parseInt(normal[1]), fullTime: 95 }
  }
  if (halftime) return { elapsed: 45, fullTime: 95 }
  // No clock: subtract 15-min halftime break if wall time exceeds 63 min
  const elapsed = wallElapsed > 63 ? wallElapsed - 15 : wallElapsed
  return { elapsed, fullTime: elapsed >= 90 ? elapsed + 5 : 95 }
}
