...
// elapsed — minutes since kickoff (float); fullTime — estimated match end (default 95)

// Parse ESPN clock like "90+7'" → fullTime = 100 (stoppage + 3-min buffer)
export function parseFullTime(clock) {
  if (!clock) return 95
  const m = clock.match(/^(\d+)\+(\d+)/)
  if (m && parseInt(m[1]) >= 90) return 90 + parseInt(m[2]) + 3
  return 95
}
...
export function getLiveStatus(needH, needA, elapsed, fullTime = 95) {
  ...
  // If match runs past our fullTime estimate (deep stoppage, no clock data yet), keep a 5-min buffer
  const effectiveFull = elapsed > fullTime ? elapsed + 5 : fullTime
  const remaining = Math.max(0, effectiveFull - elapsed)
