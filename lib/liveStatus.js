...
export function getLiveStatus(needH, needA, elapsed) {
  ...
  if (totalNeed === 1) {
    if (remaining > 30) return { label: 'Реально',   variant: 'good',    pulse: false, pct: 80 }
    if (remaining > 15) return { label: 'Можливо',   variant: 'ok',      pulse: false, pct: 70 }
    if (remaining > 5)  return { label: 'Мало часу', variant: 'warning', pulse: false, pct: 65 }
    return                     { label: 'Мало часу', variant: 'warning', pulse: false, pct: 61 }
  }

  if (totalNeed === 2) {
    if (remaining > 30) return { label: 'Можливо',      variant: 'ok',      pulse: false, pct: 50 }
    if (remaining > 15) return { label: 'Складно',      variant: 'warning', pulse: false, pct: 40 }
    if (remaining > 5)  return { label: 'Дуже складно', variant: 'danger',  pulse: false, pct: 35 }
    return                     { label: 'Дуже складно', variant: 'danger',  pulse: false, pct: 31 }
  }

  // totalNeed >= 3
  if (remaining > 30) return { label: 'Складно',         variant: 'warning',         pulse: false, pct: 25 }
  if (remaining > 15) return { label: 'Дуже складно',    variant: 'danger',          pulse: false, pct: 15 }
  if (remaining > 5)  return { label: 'Майже неможливо', variant: 'near-impossible', pulse: false, pct: 7  }
  return                     { label: 'Майже неможливо', variant: 'near-impossible', pulse: false, pct: 2  }
}
