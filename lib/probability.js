  ...
  return standings.map(s => ({
    ...
    probs: Object.fromEntries(
      Array.from({ length: N }, (_, i) => {
        const raw = ((rankFreq[s.uid][i + 1] ?? 0) / samples) * 100
        // 0 = never happened in simulation (shown as "—"); a nonzero chance that
        // rounds below 1% is kept as 0.5 so the UI can show "<1%" instead of "—".
        const pct = raw === 0 ? 0 : raw < 0.95 ? 0.5 : Math.round(raw)
        return [i + 1, pct]
      })
    ),
  }))
